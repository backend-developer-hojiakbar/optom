...
const RoleForm: React.FC<{ role?: Role; onSave: (role: Omit<Role, 'id'> | Role) => void; onClose: () => void }> = ({ role, onSave, onClose }) => {
...
const EmployeeForm: React.FC<{ employee?: Employee; onSave: (employee: Omit<Employee, 'id'> | Employee) => void; onClose: () => void }> = ({ employee, onSave, onClose }) => {
    const { roles } = useAppContext();
    const [formData, setFormData] = useState({
        name: employee?.name || '',
        phone: employee?.phone || '',
        pin: '',
        roleId: employee?.roleId || (roles.length > 0 ? roles[0].id : ''),
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!name || !phone || !roleId) {
            alert("Ism, telefon va rol maydonlari to'ldirilishi shart.");
            return;
        }
        if (!employee && pin.length !== 4) {
             alert("Yangi xodim uchun 4 xonali PIN-kod kiritilishi shart.");
             return;
        }
        ...
        const dataToSave: any = { name, phone, roleId };
        if (pin) {
            dataToSave.pin = pin;
        }
...
const Hodimlar = () => {
    const { roles, addRole, updateRole, employees, addEmployee, updateEmployee } = useAppContext();
